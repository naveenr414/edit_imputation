...
possible_values[["age"]] <- age_values
possible_values[["sex"]] <- sex_values 
possible_values[["marital"]] <- marital_values 
possible_values[["relationship"]] <- relationship_values
possible_values[["education"]] <- education_values
...
```
Let's permute some of the records randomly 
```{r}
...
write_to_csv <- function(data_list,file_name) {
  ...
  for(i in 1:length(data_list)) {
    Age <- append(data_list[[i]][["age"]],Age)
    Education <- append(data_list[[i]][["education"]],Education)
    Marital <- append(data_list[[i]][["marital"]],Marital)
    Relationship <- append(data_list[[i]][["relationship"]],Relationship)
    Sex <- append(data_list[[i]][["sex"]],Sex)
  }
  df <- data.frame(Age,Education,Marital,Relationship,Sex)
